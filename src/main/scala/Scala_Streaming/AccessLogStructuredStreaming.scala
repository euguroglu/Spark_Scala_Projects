...
object AccessLogStructuredStreaming {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession
      .builder
      .appName("AccessLogStructuredStreaming")
      .master("local[*]")
      .getOrCreate()
    ...
    val statusCountsDF = logsDF2.groupBy(col("endpoint"),
                                        window(col("eventTime"),"30 seconds", "10 seconds")).count()

    // Display the stream to the console
    val query = statusCountsDF.writeStream.outputMode("complete").format("console").queryName("counts_status").start()
